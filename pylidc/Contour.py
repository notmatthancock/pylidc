...
class Contour(Base):
    """
    The Contour model class holds a contour for a single scan slice of a Nodule object,
    as drawn by the annotating radiologist.

    inclusion: bool
        If true, the area inside the contour is included as part of the nodule.
        If false, the area inside the contour is excluded from the nodule.
    ...
    def __setattr__(self, name, value):
        if name in ['id','annotation_id','annotation','inclusion','image_z_position','dicom_file_name','coords']:
            raise ValueError("Trying to assign read-only Contour object attribute `%s` a value of `%s`." % (name,value))
        else:
            super(Contour,self).__setattr__(name,value)

    def to_matrix(self):
        """
        Return the contour-annotation coordinates as a matrix where each row contains an (x,y,z) coordinate.
        """
