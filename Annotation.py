...
_all_characteristics_ = ('subtlety', 'internalStructure', 'calcification', 'sphericity', 'margin', 'lobulation', 'spiculation', 'texture', 'malignancy')
...
class Annotation(Base):
    ...
    def __setattr__(self, name, value):
        if name in ['id','scan_id','_nodule_id','scan','subtlety','internalStructure','calcification','sphericity','margin','lobulation','spiculation','texture','malignancy']:
            raise ValueError("Trying to assign read-only Annotation object attribute `%s` a value of `%s`." % (name,value))
        else:
    ...
    def all_characteristics_as_string(self):
        ...
        for i in range(len(chars1)):
            s += ('%-18s | %-24s | %-2d' % (chars2[i], getattr(self,chars2[i])(), getattr(self,chars1[i]))) + '\n'
        return s[:-1] # cut the trailing newline character

    def all_characteristics_as_array(self):
        """
        Return all characteristic values as a numpy array in the order presented in pylidc._all_characteristics_.
        """
        return np.array([getattr(self,char) for char in _all_characteristics_])

    def bbox(self):
        """
        Return a 3 by 2 matrix, corresponding to the bounding box of the annotation within the scan. If `scan_slice` is a numpy array containing aslice of the scan, each slice of the annotation is contained within the box:

            bbox[1,0]:bbox[1,1], bbox[0,0]:bbox[0,1]

        The last row of `bbox` give the inclusive lower and upper bounds of the `image_z_position`.
        """
    ...
    def centroid(self):
        """
        Return the center of mass of the nodule as determined by its radiologist-drawn contours. Note that the first two components are the mean in image coordinates, while the last component of the centroid is the mean of `image_z_position`s.
        """
    ...
    def estimate_diameter(self, return_indices=False):
        ...
        TODO?: The greatest diameter perpendicular to the greatest diameter could be computed here as well.

        return_indices: bool, default False
            If `True`, a 3-tuple of indices is return along with the maximum diameter, `(i,j,k)`, where `i` is the index of the contour where the max occurs, and `j` and `k` refer to the two contour points between which is the maximum diameter.

        returns: float (or float,Contour)
            Returns the diameter as float, accounting for the axial plane resolution of the scan. The units are mm.
        """
    ...
    def estimate_volume(self):
        ...
        if len(self.contours) != 1:
            zvals = np.r_[zvals[0]-(zvals[1]-zvals[0]), zvals, zvals[-1]+(zvals[-1]-zvals[-2])]
        else:
            zvals = None
    ...
    def visualize_in_3d(self,**kwargs):
        """
        This method is for rough visualization only, and could look nasty if the annotated nodule has a shape that deviates significantly from spherical. Any keyword argument that is accepted by matplotlib's `plot_trisurf` function can passed as an argument to this function.

        The surface of the annotated nodule is triangulated by transforming the annotated contours into spherical coordinates, and triangulating the azimuth and zenith angles. Again, this could look quite bad if the nodule deviates from a roughly spherical shape.
        """
        fig = plt.figure(figsize=(7,7))
        ax  = fig.add_subplot(111,projection='3d')

        points = np.vstack([c.to_matrix() for c in self.contours if c.inclusion])
        ...
        triangles = Delaunay(np.c_[azimuth.flatten(),zenith.flatten()]).simplices

        # Plot the points!
        ax.plot_trisurf(points[:,0],points[:,1],points[:,2],triangles=triangles,**kwargs)
        plt.show()

    def visualize_in_scan(self, verbose=True):
        """
        Interactive visualization of the slices of the scan along with scan and annotation information. The visualization begins (but is not limited to) the first slice where the nodule occurs (according to the annotation). Contours are plotted atop the images for visualization and can be toggled on and off.
        """
        ...
        img = ax_image.imshow(images[current_slice].pixel_array, cmap=plt.cm.gray)
        ...
        # Remove the cell borders. Seems like there should be an easier way to do this...
        for cell in scan_info_table.properties()['child_artists']: cell.set_color('w')
        ...
        annotation_info_table = ax_annotation_info.table(
            cellText=[[c.capitalize() if not c.startswith('int') else 'Internal Structure', getattr(self,c.capitalize() if not c.startswith('int') else 'InternalStructure')(), getattr(self,c)] for c in _all_characteristics_],
            loc='center', cellLoc='left', colWidths=[0.45,0.45,0.1]
        )
        for cell in annotation_info_table.properties()['child_artists']: cell.set_color('w')
        ...
        contour_checkbox = CheckButtons(ax_contour_checkbox, ('Show Contours',), (True,))
        ...
        def update(_):
            ...
            sslice.label.set_text('Z: %.3f'%float(images[current_slice].ImagePositionPatient[-1]))
            if contour_checkbox.is_checked:
                for i,c in enumerate(contour_lines):
                    c.set_visible((index_of_contour[i] == current_slice) and (current_slice >= min_slice and current_slice <= max_slice))
            else:
    ...
    def contours_to_matrix(self):
        """
        Return all the contours in a 3D numpy array. Note that the first two columns are in image coordinates while the latter is given as the `image_z_position`. Thus, the image resolution is not accounted for in the first two columns.
        """
        return np.vstack([c.to_matrix() for c in self.contours])

    def as_boolean_mask(self):
        """
        Return a boolean volume which corresponds to the bounding box containing the nodule annotation. The slices of the volume are ordered by increasing `image_z_position` of the contour annotations. Note that this method doesn't account for a case where the nodule contour annotations "skip a slice". I'm not sure if this actually happens in the dataset or not. This possibility is accounted for, hoever, in the `to_volume()` method.

        returns: mask, bounding_box
            `mask` is the boolean volume. In the original 512 x 512 x num_slices dicom volume, `mask` is a boolean mask over the region, `bbox[i,0]:bbox[i,1]+1`, i=0,1,2
        ...
        # We check if these points are enclosed within each contour for a given slice.
        # `test_points` is a list of image coordinate points, offset by the bounding box.
        test_points = bbox[:2,0] + np.c_[ np.where(~mask[:,:,0]) ] # We could've used any slice of mask here.
        ...
        for contour in self.contours:
            if contour.inclusion:
                ...
                if (contour_matrix[0] != contour_matrix[-1]).all():
                    contour_matrix = np.append(contour_matrix, contour_matrix[0].reshape(1,2), axis=0)
                # Create path object and test all pixels within the contour's bounding box.
                path = mplpath.Path(contour_matrix, closed=True)
                mask[:,:,zi] = path.contains_points(test_points).reshape(mask.shape[:2])
        ...
        for contour in self.contours:
            if not contour.inclusion:
                ...
                # Create path object and test all pixels within the contour's bounding box.
                path = mplpath.Path(contour_matrix, closed=True)
                mask[:,:,zi] = np.logical_and(mask[:,:,zi], ~path.contains_points(test_points).reshape(mask.shape[:2]))
        # The first and second axes have to be swapped because of the reshape.
        return mask.swapaxes(0,1), bbox[[1,0,2]]

    def to_volume(self, new_spacing=None, pad=[(0,0),(0,0),(0,0)], verbose=False):
        """
        Return the image scan volume limited to the bounding box of the annotation. This also returns the corresponding boolean volume, corresponding to the contour annotations.

        new_spacing: float, default None
            If specified, both volumes are resampled to have voxels with the specified spacing uniformly (in mm).

        pad: list of 3 2-tuples, (pad_before,pad_after)
            Each tuple refers to the padding before and after the corresponding axes. Note that this voxel padding is done _before_ the volume is resampled.
        ...
        images = self.scan.load_all_dicom_images(verbose)
        image_zs = np.unique([float(img.ImagePositionPatient[-1]) for img in images])
        contour_zs = np.unique([c.image_z_position for c in self.contours])

        zi_start = (np.abs(bbox[2,0]-image_zs)).argmin()
        zi_stop  = (np.abs(bbox[2,1]-image_zs)).argmin()

        # This code block handles the case where the contour selfotations "skip a slice".
        if mask.shape[2] != (zi_stop-zi_start+1):
            old_mask = mask.copy()
            mask = np.zeros((old_mask.shape[0], old_mask.shape[1], zi_stop-zi_start+1), dtype=np.bool)
            z_to_index = dict(zip(image_zs[zi_start:zi_stop+1], range(image_zs[zi_start:zi_stop+1].shape[0])))
        ...
        if zi_start-pad[2][0] < 0 or zi_stop+pad[2][1] >= len(images):
            raise IndexError, 'Specified z-padding results in out-of-bounds index.'

        # Add padding to the indices and trim the images / image_zs.
        zi_start -= pad[2][0]
        zi_stop  += pad[2][1]
        images   = images[  zi_start:zi_stop+1]
        image_zs = image_zs[zi_start:zi_stop+1]
        ...
        if int(bbox[0,0])-int(pad[0][0]) < 0 or int(bbox[0,1])+1+int(pad[0][1]) >= 512:
            raise IndexError, 'Specified x-padding results in out-of-bounds index.'
        if int(bbox[1,0])-int(pad[1][0]) < 0 or int(bbox[1,1])+1+int(pad[1][1]) >= 512:
            raise IndexError, 'Specified y-padding results in out-of-bounds index.'
        ...
        if new_spacing is None:
            ...
            z_points_new = np.arange(z_points.min(), z_points.max(), new_spacing)

            x,y,z = np.meshgrid(x_points_new, y_points_new, z_points_new, indexing='ij')
    ...
    def _as_set(self):
        """
        Private function used to computed overlap between nodules of the same scan. This function returns a set where is element is a 3-tuple referring to a voxel within the scan. If the voxel is in the set, the nodule is considered to be defined there. Essentially this is a boolean mask stored as a set for memory efficiency.
        """
        included = set()
        excluded = set()
        ...
        for contour in self.contours:
            ...
            points_in_contour = np.c_[ points_in_contour, np.ones(points_in_contour.shape[0])*contour.image_z_position ]

            # Now turn the numpy matrix into a list of tuples, so we can add it to the corresponding set.
...
Scan.annotations = relationship('Annotation', order_by=Annotation.id, back_populates='scan')
