...
class Scan(Base):
    ...
    Attributes:
        ...
        slice_thickness: float
            Dicom attribute, `(0018,0050)`. Note that this may not be equal to `spacing_betwee_slices`.

        pixel_spacing: float
            Dicom attribute, `(0028,0030)`. This is normally two values. All scans in the LIDC have equal resolutions in the transverse plane, so only one value is taken here.
        ...
        sorted_dicom_file_names: string
            This is a string containing a comma-separated list like `[number].dcm`. It is a list of the dicom file names for this scan in order of increasing z-coordinate of dicom attribute, `(0020,0032)`. In rare cases where a scan includes multiple files with the same z-coordinate, the one with the lesser `InstanceNumber` is used.

    Example:
        >>> import pylidc as pl
        >>> q = pl.query(pl.Scan).filter(pl.Scan.resolution_z <= 1)
        >>> print q.count()
        >>> # => 97
        >>> scan = q.first()
    ...
    def __setattr__(self, name, value):
        if name in ['id','study_instance_uid','series_instance_uid','patient_id','slice_thickness','pixel_spacing','contrast_used','is_from_initial', 'sorted_dicom_file_names']:
            raise ValueError("Trying to assign read-only Scan object attribute `%s` a value of `%s`." % (name,value))
        else:
            super(Scan,self).__setattr__(name,value)
    
    def get_path_to_dicom_files(self):
        """
        Get the path to where the dicom files are stored for this scan, relative to the root path set in the main module `pylidc.set_path_to_dicom_files()` function.
        ...
        engine  = create_engine('sqlite:///'+ os.path.join(module_path,'db','pylidc.sqlite'))
        session = sessionmaker(bind=engine)()
        scan_root_path = session.query(_Configuration).filter(_Configuration.key == 'path_to_dicom_files').first().value
        session.close()
        return os.path.join(scan_root_path, self.patient_id, self.study_instance_uid, self.series_instance_uid)

    def annotations_with_matching_overlap(self, tol=0.5, return_overlap_scores=False):
        """
        TODO: This function should ideally match match nodule annotations not just on overlap, but on centroid distance, too.

        Find which annotations refer to the same nodule. This method clusters nodule annotations for this scan by comparing the overlap between contour annotations. Specifically, a pairwise distance between annotation[i] with annotation[j] is formed by computing:
        
        1 - ( size_of_intersection( annotation[i], annotation[j] ) / size_of_union( annotation[i], annotation[j] ) ).

        tol: float, default 0.5
            A value between 0 and 1. The tolerance whereby annotations are grouped. A high tolerance means annotations with less overlap are grouped. A low tolerance requires that annotations must have high overlap to be grouped.

        returns: clusters, a list of nodule groups.
            `clusters[j]` is a list of nodule annotations grouped by above. It should be the case that `len(clusters[j])` is always be between 1 and 4 because a max of 4 radiologists could have annotated a single nodule. `len(clusters)` is the number of unique nodules present in this scan as determined by this overlap method and the tolerance used.
        """
        ...
        for i in range(N):
            for j in range(i,N):
                if i==j:
                    ...
                    A[i,j] = len(sets[i].intersection(sets[j])) / float(len(sets[i].union(sets[j])))
                    A[j,i] = A[i,j]
        D = 1-A # This converts the affinity matrix to a symmetric distance matrix.
        # Cluster using the overlap distance.
        cluster_labels = hierarchy.fcluster(hierarchy.linkage(D), tol, 'distance')
        clusters = []
        for label in range(1,cluster_labels.max()+1): # indexing for hierachy.fcluster starts at 1.
            clusters.append([])
    ...
    def load_all_dicom_images(self, verbose=True):
        ...
        assert os.path.exists(path), "The path:\n\n %s \n\n doesn't exist. Does the folder exists there? Have you set set the root path to your dicom folder, using `pylidc.set_path_to_dicom_files()`?" % path 

        if verbose: print("Loading dicom files ... This may take a moment.")

        fnames = [fname for fname in os.listdir(path) if fname.endswith('.dcm')]
        sorted_fnames = self.sorted_dicom_file_names.split(',')
        
        # Some sets have the dicom files padded with zeros in front, some don't apparently.
    ...
    def visualize(self):
        ...
        TODO: Matching nodule annnotations could be viewed on top of the scan as well.
        """
        ...
        sslice  = Slider(ax_slice, 'Z: %.3f'%float(images[current_slice].ImagePositionPatient[-1]), 0, len(images)-1, valinit=current_slice, valfmt=u'Slice: %d')
